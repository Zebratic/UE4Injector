...
namespace utils {
	BOOL Setup() {
		// load ntdll library so we can manipulate our game process
		auto module = LoadLibrary(L"ntdll.dll");
		if (!module) {
			errorf("Handle for NTDLL failed\n");
			return FALSE;
		}

		// Find our module's address
	...
	// The rest down here is just normal utility functions used across the source code :)
#pragma region Process Manipulation
